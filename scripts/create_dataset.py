...
for dset in ['val']:  # ['train', 'test', 'val']:
    ...
    tag_ids = factory_metadata[factory_metadata.tag_type == "DV"].tag_id.tolist() + factory_metadata[factory_metadata.tag_type == "MV"].tag_id.tolist() + factory_metadata[factory_metadata.tag_type == "CV"].tag_id.tolist()
    ...
    def multivar_example_gen_func() -> Generator[dict[str, Any], None, None]:
        for i, timestamp in y_timestamps['timestamp'].items():
            ...
            if len(example) < (context_length + prediction_length):
                continue
            yield {
                "target": example[tag_ids].to_numpy().T,  # array of shape (var, time)
    ...
    hf_dataset.save_to_disk(os.path.join(base_dir,f"dataset/{dset}_small"))
